...
<?php endif; ?>
	...
	<?php if ( 'open' == $post->comment_status ) : ?>
		<?php comment_form(
			array(
				'label_submit' => esc_attr__( 'Post Comment', 'extra' ),
				'comment_field' => '<p class="comment-form-comment"><label for="comment">'
					. esc_html_x( 'Comente', 'noun', 'extra' )
					. '</label><textarea id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea></p>',
				'title_reply' => '<span>' .
					esc_attr__( 'Envie seu comentário', 'extra' ) 
					. '</span>',
				'title_reply_to' => esc_attr__( 'Envie seu comentário para %s', 'extra' )
			)
		); ?>
	<?php else: ?>
