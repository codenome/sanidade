...
function extra_rating_stars_display( $post_id = 0 ) {
	...
	if ( $rating = extra_get_user_post_rating( $post_id ) ) {
		$output = '<p id="rate-title" class="rate-title">' . esc_html__( 'Sua classificação:', 'extra' ) . '</p>';
		$output .= '<div id="rated-stars">' . extra_make_fixed_stars( $rating ) . '</div>';

	} else {
		$title = esc_html__( 'Classifique:', 'extra' );

		$output = '<p id="rate-title" class="rate-title">' . esc_html__( 'Classifique:', 'extra' ) . '</p>';
...
function extra_header_search_field( $echo = true ) {
	$output = sprintf(
		...
		esc_url( home_url( '/' ) ),
		esc_attr_x( 'Buscar', 'placeholder', 'extra' ),
		get_search_query(),
		esc_attr_x( 'Pesquisar por:', 'label', 'extra' )
	);
